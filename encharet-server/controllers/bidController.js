...
bidControllers.createBid = async (req, res) => {
  ...
  const { id, status } = req.user.newUser ? req.user.newUser : req.user;
  const { cpo_amount, title, description, fee, deadline, tag } = req.body;
...
bidControllers.browseBids = async (req, res) => {
  ...
  try {
    const bids = await prisma.bid.findMany({
      ...
      orderBy: {
        created_at: date === "latest" ? "desc" : "asc" || undefined,
      },
      include: {
        BidFiles: true,
      },
...
bidControllers.getOngoingBids = async (req, res) => {
  ...
  try {
    ...
    applications.map(async (application) => {
      bids.push(
        await prisma.bid.findMany({ where: { id: application.bid_id } })[0]
      );
    });
...
bidControllers.getUpcomingBids = async (req, res) => {
  ...
  try {
    const upcomingBids = await prisma.bid.findMany({
      where: {
        status: "PENDING",
      },
      orderBy: {
