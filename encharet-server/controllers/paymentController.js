...
paymentControllers.requestWithdraw = async(req, res) => {
    const {id, fname, lname, phonenumber} = req.user.newUser
        ? req.user.newUser
        : req.user;
    ...
    try {
        ...
        const no_of_request = await prisma
            .withdrawalRequest
            .count({
                where: {
                    user_id: id,
        ...
        if (!exists) {
            if (user[0].status === "ACTIVE") {
                if (user[0].active_balance >= amount) {
                    const WithdrawalRequest = await prisma
                        .withdrawalRequest
                        .create({
                            data: {
                                user_id: id,
                    ...
                    return res
                        .status(401)
                        .send({status: 401, message: "Insufficient balance to withdraw!!"});
                }
            } else {
                return res
                    .status(401)
            ...
            return res
                .status(401)
                .send({status: 401, message: "You already have another pending withdrawal request, please wait for response!!"});
        }
    } catch (error) {
        console.log(error);
        return res
