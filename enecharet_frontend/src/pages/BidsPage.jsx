...
const BidsPage = () => {
    ...
    const [bidTag, setBidTag] = useState("");
    ...
    useEffect(() => {
        ...
        const fetchTypedData = async () => {
            ...
            if (bidType !==null && bidType !=="" && bidType !== undefined) {
                requestString += `status=${bidType}`;
            }
            console.log(requestString);
            axios.get(BASE_URL + requestString, { headers: header })
                .then(response => {
                    setIsLoading(false);
        ...
        const fetchTypedDataOngoing = async () => {
            let requestString = "/api/bids/ongoing?";
            console.log("in on going");
            ...
            if (bidType !==null && bidType !=="" && bidType !== undefined) {
                requestString += `status=PENDING`;
            }
            console.log(requestString);
            axios.get(BASE_URL + requestString, { headers: header })
                .then(response => {
                    setIsLoading(false);
                    setBidsList(response.data.flat());
                })
                .catch(err => {
                    setIsLoading(false);
                    alert(err);
        ...
        if (token && bidType != "ONGOING") {
            fetchTypedData();
        } 
        else if(token && bidType == "ONGOING"){
            fetchTypedDataOngoing();
        }
        else {
            fetchData();
        }
        ...
        <div className="m-auto sm:my-4">
            ...
            <div className="lg:px-40 md:px-20 sm:px-10 my-7">
                <div className="flex flex-row justify-center">
                    <Button.Group>
                        <Button color="gray" onClick={() => setBidType("COMPLETED")}>
                            Closed
                        </Button>
                        <Button color="gray" onClick={() => setBidType("PENDING")}>
                            Upcoming
                        </Button>
